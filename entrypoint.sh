...
if [[ -z $RUNNER_TOKEN && -z $PAT_ACCESS_TOKEN ]]; then
    echo "Error : You need to set RUNNER_TOKEN (or PAT_ACCESS_TOKEN) environment variable."
    exit 1
fi
...
if [[ -f ".runner" ]]; then
    ...
    if [[ -n $PAT_ACCESS_TOKEN ]]; then
        ...
        RUNNER_TOKEN="$(curl -XPOST -fsSL \
            -H "Authorization: token ${PAT_ACCESS_TOKEN}" \
